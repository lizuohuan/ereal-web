<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<?mso-application progid="Excel.Sheet"?>
<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet" xmlns:o="urn:schemas-microsoft-com:office:office"
          xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet"
          xmlns:html="http://www.w3.org/TR/REC-html40" xmlns:dt="uuid:C2F41010-65B3-11d1-A29F-00AA00C14882">
    <DocumentProperties xmlns="urn:schemas-microsoft-com:office:office">
        <Author>Administrator</Author>
        <Created>2017-07-11T09:13:35Z</Created>
        <LastSaved>2017-07-11T10:14:56Z</LastSaved>
    </DocumentProperties>
    <CustomDocumentProperties xmlns="urn:schemas-microsoft-com:office:office">
        <KSOProductBuildVer dt:dt="string">2052-10.1.0.6554</KSOProductBuildVer>
    </CustomDocumentProperties>
    <ExcelWorkbook xmlns="urn:schemas-microsoft-com:office:excel">
        <WindowWidth>27765</WindowWidth>
        <WindowHeight>13650</WindowHeight>
        <ProtectStructure>False</ProtectStructure>
        <ProtectWindows>False</ProtectWindows>
    </ExcelWorkbook>
    <Styles>
        <Style ss:ID="Default" ss:Name="Normal">
            <Alignment/>
            <Borders/>
            <Font ss:FontName="宋体" x:CharSet="134" ss:Size="12"/>
            <Interior/>
            <NumberFormat/>
            <Protection/>
        </Style>
        <Style ss:ID="s24" ss:Name="输出">
            <Borders>
                <Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#3F3F3F"/>
                <Border ss:Position="Left" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#3F3F3F"/>
                <Border ss:Position="Right" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#3F3F3F"/>
                <Border ss:Position="Top" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#3F3F3F"/>
            </Borders>
            <Font ss:FontName="宋体" x:CharSet="0" ss:Size="11" ss:Color="#3F3F3F" ss:Bold="1"/>
            <Interior ss:Color="#F2F2F2" ss:Pattern="Solid"/>
        </Style>
        <Style ss:ID="s17" ss:Name="标题">
            <Font ss:FontName="宋体" x:CharSet="134" ss:Size="18" ss:Color="#44546A" ss:Bold="1"/>
        </Style>
        <Style ss:ID="s1" ss:Name="货币[0]">
            <NumberFormat
                    ss:Format="_ &quot;￥&quot;* #,##0_ ;_ &quot;￥&quot;* \-#,##0_ ;_ &quot;￥&quot;* &quot;-&quot;_ ;_ @_ "/>
        </Style>
        <Style ss:ID="s8" ss:Name="千位分隔">
            <NumberFormat ss:Format="_ * #,##0.00_ ;_ * \-#,##0.00_ ;_ * &quot;-&quot;??_ ;_ @_ "/>
        </Style>
        <Style ss:ID="s20" ss:Name="标题 2">
            <Borders>
                <Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="2" ss:Color="#5B9BD5"/>
            </Borders>
            <Font ss:FontName="宋体" x:CharSet="134" ss:Size="13" ss:Color="#44546A" ss:Bold="1"/>
        </Style>
        <Style ss:ID="s13" ss:Name="注释">
            <Borders>
                <Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#B2B2B2"/>
                <Border ss:Position="Left" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#B2B2B2"/>
                <Border ss:Position="Right" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#B2B2B2"/>
                <Border ss:Position="Top" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#B2B2B2"/>
            </Borders>
            <Interior ss:Color="#FFFFCC" ss:Pattern="Solid"/>
        </Style>
        <Style ss:ID="s5" ss:Name="千位分隔[0]">
            <NumberFormat ss:Format="_ * #,##0_ ;_ * \-#,##0_ ;_ * &quot;-&quot;_ ;_ @_ "/>
        </Style>
        <Style ss:ID="s25" ss:Name="计算">
            <Borders>
                <Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#7F7F7F"/>
                <Border ss:Position="Left" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#7F7F7F"/>
                <Border ss:Position="Right" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#7F7F7F"/>
                <Border ss:Position="Top" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#7F7F7F"/>
            </Borders>
            <Font ss:FontName="宋体" x:CharSet="0" ss:Size="11" ss:Color="#FA7D00" ss:Bold="1"/>
            <Interior ss:Color="#F2F2F2" ss:Pattern="Solid"/>
        </Style>
        <Style ss:ID="s18" ss:Name="解释性文本">
            <Font ss:FontName="宋体" x:CharSet="0" ss:Size="11" ss:Color="#7F7F7F" ss:Italic="1"/>
        </Style>
        <Style ss:ID="s10" ss:Name="超链接">
            <Font ss:FontName="宋体" x:CharSet="0" ss:Size="11" ss:Color="#0000FF" ss:Underline="Single"/>
        </Style>
        <Style ss:ID="s4" ss:Name="货币">
            <NumberFormat
                    ss:Format="_ &quot;￥&quot;* #,##0.00_ ;_ &quot;￥&quot;* \-#,##0.00_ ;_ &quot;￥&quot;* &quot;-&quot;??_ ;_ @_ "/>
        </Style>
        <Style ss:ID="s3" ss:Name="输入">
            <Borders>
                <Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#7F7F7F"/>
                <Border ss:Position="Left" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#7F7F7F"/>
                <Border ss:Position="Right" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#7F7F7F"/>
                <Border ss:Position="Top" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#7F7F7F"/>
            </Borders>
            <Font ss:FontName="宋体" x:CharSet="0" ss:Size="11" ss:Color="#3F3F76"/>
            <Interior ss:Color="#FFCC99" ss:Pattern="Solid"/>
        </Style>
        <Style ss:ID="s6" ss:Name="40% - 强调文字颜色 3">
            <Font ss:FontName="宋体" x:CharSet="0" ss:Size="11" ss:Color="#000000"/>
            <Interior ss:Color="#DBDBDB" ss:Pattern="Solid"/>
        </Style>
        <Style ss:ID="s2" ss:Name="20% - 强调文字颜色 3">
            <Font ss:FontName="宋体" x:CharSet="0" ss:Size="11" ss:Color="#000000"/>
            <Interior ss:Color="#EDEDED" ss:Pattern="Solid"/>
        </Style>
        <Style ss:ID="s7" ss:Name="差">
            <Font ss:FontName="宋体" x:CharSet="0" ss:Size="11" ss:Color="#9C0006"/>
            <Interior ss:Color="#FFC7CE" ss:Pattern="Solid"/>
        </Style>
        <Style ss:ID="s9" ss:Name="60% - 强调文字颜色 3">
            <Font ss:FontName="宋体" x:CharSet="0" ss:Size="11" ss:Color="#FFFFFF"/>
            <Interior ss:Color="#C9C9C9" ss:Pattern="Solid"/>
        </Style>
        <Style ss:ID="s26" ss:Name="检查单元格">
            <Borders>
                <Border ss:Position="Bottom" ss:LineStyle="Double" ss:Weight="3" ss:Color="#3F3F3F"/>
                <Border ss:Position="Left" ss:LineStyle="Double" ss:Weight="3" ss:Color="#3F3F3F"/>
                <Border ss:Position="Right" ss:LineStyle="Double" ss:Weight="3" ss:Color="#3F3F3F"/>
                <Border ss:Position="Top" ss:LineStyle="Double" ss:Weight="3" ss:Color="#3F3F3F"/>
            </Borders>
            <Font ss:FontName="宋体" x:CharSet="0" ss:Size="11" ss:Color="#FFFFFF" ss:Bold="1"/>
            <Interior ss:Color="#A5A5A5" ss:Pattern="Solid"/>
        </Style>
        <Style ss:ID="s19" ss:Name="标题 1">
            <Borders>
                <Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="2" ss:Color="#5B9BD5"/>
            </Borders>
            <Font ss:FontName="宋体" x:CharSet="134" ss:Size="15" ss:Color="#44546A" ss:Bold="1"/>
        </Style>
        <Style ss:ID="s12" ss:Name="已访问的超链接">
            <Font ss:FontName="宋体" x:CharSet="0" ss:Size="11" ss:Color="#800080" ss:Underline="Single"/>
        </Style>
        <Style ss:ID="s11" ss:Name="百分比">
            <NumberFormat ss:Format="0%"/>
        </Style>
        <Style ss:ID="s23" ss:Name="60% - 强调文字颜色 4">
            <Font ss:FontName="宋体" x:CharSet="0" ss:Size="11" ss:Color="#FFFFFF"/>
            <Interior ss:Color="#FFD966" ss:Pattern="Solid"/>
        </Style>
        <Style ss:ID="s14" ss:Name="60% - 强调文字颜色 2">
            <Font ss:FontName="宋体" x:CharSet="0" ss:Size="11" ss:Color="#FFFFFF"/>
            <Interior ss:Color="#F4B084" ss:Pattern="Solid"/>
        </Style>
        <Style ss:ID="s15" ss:Name="标题 4">
            <Font ss:FontName="宋体" x:CharSet="134" ss:Size="11" ss:Color="#44546A" ss:Bold="1"/>
        </Style>
        <Style ss:ID="s22" ss:Name="标题 3">
            <Borders>
                <Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="2" ss:Color="#ACCCEA"/>
            </Borders>
            <Font ss:FontName="宋体" x:CharSet="134" ss:Size="11" ss:Color="#44546A" ss:Bold="1"/>
        </Style>
        <Style ss:ID="s16" ss:Name="警告文本">
            <Font ss:FontName="宋体" x:CharSet="0" ss:Size="11" ss:Color="#FF0000"/>
        </Style>
        <Style ss:ID="s46" ss:Name="强调文字颜色 6">
            <Font ss:FontName="宋体" x:CharSet="0" ss:Size="11" ss:Color="#FFFFFF"/>
            <Interior ss:Color="#70AD47" ss:Pattern="Solid"/>
        </Style>
        <Style ss:ID="s21" ss:Name="60% - 强调文字颜色 1">
            <Font ss:FontName="宋体" x:CharSet="0" ss:Size="11" ss:Color="#FFFFFF"/>
            <Interior ss:Color="#9BC2E6" ss:Pattern="Solid"/>
        </Style>
        <Style ss:ID="s28" ss:Name="强调文字颜色 2">
            <Font ss:FontName="宋体" x:CharSet="0" ss:Size="11" ss:Color="#FFFFFF"/>
            <Interior ss:Color="#ED7D31" ss:Pattern="Solid"/>
        </Style>
        <Style ss:ID="s27" ss:Name="20% - 强调文字颜色 6">
            <Font ss:FontName="宋体" x:CharSet="0" ss:Size="11" ss:Color="#000000"/>
            <Interior ss:Color="#E2EFDA" ss:Pattern="Solid"/>
        </Style>
        <Style ss:ID="s29" ss:Name="链接单元格">
            <Borders>
                <Border ss:Position="Bottom" ss:LineStyle="Double" ss:Weight="3" ss:Color="#FF8001"/>
            </Borders>
            <Font ss:FontName="宋体" x:CharSet="0" ss:Size="11" ss:Color="#FA7D00"/>
        </Style>
        <Style ss:ID="s30" ss:Name="汇总">
            <Borders>
                <Border ss:Position="Bottom" ss:LineStyle="Double" ss:Weight="3" ss:Color="#5B9BD5"/>
                <Border ss:Position="Top" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#5B9BD5"/>
            </Borders>
            <Font ss:FontName="宋体" x:CharSet="0" ss:Size="11" ss:Color="#000000" ss:Bold="1"/>
        </Style>
        <Style ss:ID="s31" ss:Name="好">
            <Font ss:FontName="宋体" x:CharSet="0" ss:Size="11" ss:Color="#006100"/>
            <Interior ss:Color="#C6EFCE" ss:Pattern="Solid"/>
        </Style>
        <Style ss:ID="s32" ss:Name="适中">
            <Font ss:FontName="宋体" x:CharSet="0" ss:Size="11" ss:Color="#9C6500"/>
            <Interior ss:Color="#FFEB9C" ss:Pattern="Solid"/>
        </Style>
        <Style ss:ID="s34" ss:Name="强调文字颜色 1">
            <Font ss:FontName="宋体" x:CharSet="0" ss:Size="11" ss:Color="#FFFFFF"/>
            <Interior ss:Color="#5B9BD5" ss:Pattern="Solid"/>
        </Style>
        <Style ss:ID="s33" ss:Name="20% - 强调文字颜色 5">
            <Font ss:FontName="宋体" x:CharSet="0" ss:Size="11" ss:Color="#000000"/>
            <Interior ss:Color="#D9E1F2" ss:Pattern="Solid"/>
        </Style>
        <Style ss:ID="s44" ss:Name="40% - 强调文字颜色 5">
            <Font ss:FontName="宋体" x:CharSet="0" ss:Size="11" ss:Color="#000000"/>
            <Interior ss:Color="#B4C6E7" ss:Pattern="Solid"/>
        </Style>
        <Style ss:ID="s35" ss:Name="20% - 强调文字颜色 1">
            <Font ss:FontName="宋体" x:CharSet="0" ss:Size="11" ss:Color="#000000"/>
            <Interior ss:Color="#DDEBF7" ss:Pattern="Solid"/>
        </Style>
        <Style ss:ID="s45" ss:Name="60% - 强调文字颜色 5">
            <Font ss:FontName="宋体" x:CharSet="0" ss:Size="11" ss:Color="#FFFFFF"/>
            <Interior ss:Color="#8EA9DB" ss:Pattern="Solid"/>
        </Style>
        <Style ss:ID="s36" ss:Name="40% - 强调文字颜色 1">
            <Font ss:FontName="宋体" x:CharSet="0" ss:Size="11" ss:Color="#000000"/>
            <Interior ss:Color="#BDD7EE" ss:Pattern="Solid"/>
        </Style>
        <Style ss:ID="s41" ss:Name="20% - 强调文字颜色 4">
            <Font ss:FontName="宋体" x:CharSet="0" ss:Size="11" ss:Color="#000000"/>
            <Interior ss:Color="#FFF2CC" ss:Pattern="Solid"/>
        </Style>
        <Style ss:ID="s37" ss:Name="20% - 强调文字颜色 2">
            <Font ss:FontName="宋体" x:CharSet="0" ss:Size="11" ss:Color="#000000"/>
            <Interior ss:Color="#FCE4D6" ss:Pattern="Solid"/>
        </Style>
        <Style ss:ID="s42" ss:Name="40% - 强调文字颜色 4">
            <Font ss:FontName="宋体" x:CharSet="0" ss:Size="11" ss:Color="#000000"/>
            <Interior ss:Color="#FFE699" ss:Pattern="Solid"/>
        </Style>
        <Style ss:ID="s38" ss:Name="40% - 强调文字颜色 2">
            <Font ss:FontName="宋体" x:CharSet="0" ss:Size="11" ss:Color="#000000"/>
            <Interior ss:Color="#F8CBAD" ss:Pattern="Solid"/>
        </Style>
        <Style ss:ID="s43" ss:Name="强调文字颜色 5">
            <Font ss:FontName="宋体" x:CharSet="0" ss:Size="11" ss:Color="#FFFFFF"/>
            <Interior ss:Color="#4472C4" ss:Pattern="Solid"/>
        </Style>
        <Style ss:ID="s39" ss:Name="强调文字颜色 3">
            <Font ss:FontName="宋体" x:CharSet="0" ss:Size="11" ss:Color="#FFFFFF"/>
            <Interior ss:Color="#A5A5A5" ss:Pattern="Solid"/>
        </Style>
        <Style ss:ID="s40" ss:Name="强调文字颜色 4">
            <Font ss:FontName="宋体" x:CharSet="0" ss:Size="11" ss:Color="#FFFFFF"/>
            <Interior ss:Color="#FFC000" ss:Pattern="Solid"/>
        </Style>
        <Style ss:ID="s47" ss:Name="40% - 强调文字颜色 6">
            <Font ss:FontName="宋体" x:CharSet="0" ss:Size="11" ss:Color="#000000"/>
            <Interior ss:Color="#C6E0B4" ss:Pattern="Solid"/>
        </Style>
        <Style ss:ID="s48" ss:Name="60% - 强调文字颜色 6">
            <Font ss:FontName="宋体" x:CharSet="0" ss:Size="11" ss:Color="#FFFFFF"/>
            <Interior ss:Color="#A9D08E" ss:Pattern="Solid"/>
        </Style>
        <Style ss:ID="s49">
            <Alignment/>
            <Borders/>
            <Font ss:FontName="宋体" x:CharSet="134" ss:Size="12"/>
            <Interior/>
            <NumberFormat/>
            <Protection/>
        </Style>
        <Style ss:ID="s50">
            <Alignment/>
            <Borders/>
            <Font ss:FontName="宋体" x:CharSet="134" ss:Size="12"/>
            <Interior/>
            <NumberFormat ss:Format="yyyy&quot;年&quot;m&quot;月&quot;;@"/>
            <Protection/>
        </Style>
        <Style ss:ID="s51">
            <Alignment ss:Horizontal="Center"/>
        </Style>
        <Style ss:ID="s52">
            <Alignment ss:Horizontal="Center"/>
            <NumberFormat ss:Format="yyyy&quot;年&quot;m&quot;月&quot;;@"/>
        </Style>
        <Style ss:ID="s53">
            <Alignment ss:Horizontal="Center"/>
            <Borders/>
            <Font ss:FontName="宋体" x:CharSet="134" ss:Size="12"/>
            <Interior/>
            <NumberFormat/>
            <Protection/>
        </Style>
        <Style ss:ID="s54">
            <Alignment/>
            <Borders/>
            <Font ss:FontName="宋体" x:CharSet="134" ss:Size="12"/>
            <Interior/>
            <NumberFormat ss:Format="0%"/>
            <Protection/>
        </Style>
        <Style ss:ID="s55">
            <Alignment ss:Horizontal="Center"/>
        </Style>
        <Style ss:ID="s56">
            <Alignment ss:Horizontal="Center"/>
        </Style>
        <Style ss:ID="s57">
            <Alignment ss:Horizontal="Center"/>
        </Style>
        <Style ss:ID="s58">
            <Alignment ss:Horizontal="Center"/>
        </Style>
        <Style ss:ID="s59">
            <Alignment ss:Horizontal="Center"/>
        </Style>
        <Style ss:ID="s60">
            <Alignment ss:Horizontal="Center"/>
        </Style>
        <Style ss:ID="s61">
            <Alignment ss:Horizontal="Center"/>
        </Style>
        <Style ss:ID="s62">
            <Alignment ss:Horizontal="Center"/>
        </Style>
        <Style ss:ID="s63">
            <Alignment ss:Horizontal="Center"/>
        </Style>
        <Style ss:ID="s64">
            <Alignment ss:Horizontal="Center"/>
        </Style>
        <Style ss:ID="s65">
            <Alignment ss:Horizontal="Center"/>
        </Style>
        <Style ss:ID="s66">
            <Alignment ss:Horizontal="Center"/>
        </Style>
        <Style ss:ID="s67">
            <Alignment ss:Horizontal="Center"/>
            <Borders/>
            <Font ss:FontName="宋体" x:CharSet="134" ss:Size="12"/>
            <Interior/>
            <NumberFormat/>
            <Protection/>
        </Style>
        <Style ss:ID="s68">
            <Alignment ss:Horizontal="Center"/>
        </Style>
        <Style ss:ID="s69">
            <Alignment ss:Horizontal="Center"/>
        </Style>
        <Style ss:ID="s70">
            <Alignment ss:Horizontal="Center"/>
            <NumberFormat ss:Format="yyyy&quot;年&quot;m&quot;月&quot;;@"/>
        </Style>
        <Style ss:ID="s71">
            <Alignment ss:Horizontal="Center"/>
        </Style>
        <Style ss:ID="s72">
            <Alignment ss:Horizontal="Center"/>
        </Style>
        <Style ss:ID="s73">
            <Alignment ss:Horizontal="Center"/>
        </Style>
        <Style ss:ID="s74">
            <Alignment ss:Horizontal="Center"/>
        </Style>
        <Style ss:ID="s75">
            <Alignment ss:Horizontal="Center"/>
        </Style>
        <Style ss:ID="s76">
            <Alignment ss:Horizontal="Center"/>
        </Style>
        <Style ss:ID="s77">
            <Alignment ss:Horizontal="Center"/>
        </Style>
        <Style ss:ID="s78">
            <Alignment ss:Horizontal="Center"/>
        </Style>
        <Style ss:ID="s79">
            <Alignment ss:Horizontal="Center"/>
        </Style>
        <Style ss:ID="s80">
            <Alignment ss:Horizontal="Center"/>
        </Style>
        <Style ss:ID="s81">
            <Alignment ss:Horizontal="Center"/>
        </Style>
    </Styles>
    <Worksheet ss:Name="Sheet1">
        <Row ss:ExpandedColumnCount="35" ss:ExpandedRowCount="7" x:FullColumns="1" x:FullRows="1"
               ss:DefaultColumnWidth="54" ss:DefaultRowHeight="14.25">
            <Column ss:Index="4" ss:StyleID="s50" ss:Width="62.25"/>
            <Column ss:Index="7" ss:StyleID="s49" ss:AutoFitWidth="0" ss:Width="69"/>
            <Column ss:StyleID="s49" ss:AutoFitWidth="0" ss:Width="129"/>
            <Column ss:StyleID="s49" ss:AutoFitWidth="0" ss:Width="69"/>
            <Column ss:Index="22" ss:StyleID="s49" ss:AutoFitWidth="0" ss:Width="32.25"/>
            <Column ss:StyleID="s49" ss:AutoFitWidth="0" ss:Width="26.25"/>
            <Row ss:Height="27">
                <Cell ss:StyleID="s55" ss:MergeAcross="5">
                    <Data ss:Type="String">一真咨询2016年内部项目台帐</Data>
                </Cell>
            </Row>
            <Row ss:Height="27">
                <Cell ss:Index="2" ss:StyleID="s56" ss:MergeAcross="4">
                    <Data ss:Type="String">项目立项信息</Data>
                </Cell>
                <Cell ss:StyleID="s57" ss:MergeAcross="2"/>
                <Cell ss:StyleID="s58" ss:MergeAcross="13">
                    <Data ss:Type="String">项目阶段验收结果</Data>
                </Cell>
                <Cell ss:StyleID="s59" ss:MergeAcross="11">
                    <Data ss:Type="String">第一次验收</Data>
                </Cell>
            </Row>
            <Row>
                <Cell ss:StyleID="s66" ss:MergeDown="1">
                    <Data ss:Type="String">团队</Data>
                </Cell>
                <Cell ss:StyleID="s68" ss:MergeDown="1">
                    <Data ss:Type="String">任务序号</Data>
                </Cell>
                <Cell ss:StyleID="s69" ss:MergeDown="1">
                    <Data ss:Type="String">编号</Data>
                </Cell>
                <Cell ss:StyleID="s70" ss:MergeDown="1">
                    <Data ss:Type="String">立项月份</Data>
                </Cell>
                <Cell ss:StyleID="s71" ss:MergeDown="1">
                    <Data ss:Type="String">是否建立</Data>
                </Cell>
                <Cell ss:StyleID="s72" ss:MergeDown="1">
                    <Data ss:Type="String">项目名称</Data>
                </Cell>
                <Cell ss:StyleID="s73" ss:MergeDown="1">
                    <Data ss:Type="String">初始工作量</Data>
                </Cell>
                <Cell ss:StyleID="s74" ss:MergeDown="1">
                    <Data ss:Type="String">直接责任人/项目经理</Data>
                </Cell>
                <Cell ss:StyleID="s75" ss:MergeDown="1">
                    <Data ss:Type="String">直接汇报人</Data>
                </Cell>
                <Cell ss:StyleID="s76" ss:MergeDown="1">
                    <Data ss:Type="String">当前进度</Data>
                </Cell>
                <Cell ss:StyleID="s60" ss:MergeAcross="4">
                    <Data ss:Type="String">质量得分</Data>
                </Cell>
                <Cell ss:StyleID="s77" ss:MergeDown="1">
                    <Data ss:Type="String">类型</Data>
                </Cell>
                <Cell ss:StyleID="s78" ss:MergeDown="1">
                    <Data ss:Type="String">质量系数</Data>
                </Cell>
                <Cell ss:StyleID="s61" ss:MergeAcross="2">
                    <Data ss:Type="String">项目经理</Data>
                </Cell>
                <Cell ss:StyleID="s62" ss:MergeAcross="2">
                    <Data ss:Type="String">项目成员</Data>
                </Cell>
                <Cell ss:StyleID="s79" ss:MergeDown="1">
                    <Data ss:Type="String">验收时间</Data>
                </Cell>
                <Cell ss:StyleID="s80" ss:MergeDown="1">
                    <Data ss:Type="String">验收人</Data>
                </Cell>
                <Cell ss:StyleID="s81" ss:MergeDown="1">
                    <Data ss:Type="String">总体进度</Data>
                </Cell>
                <Cell ss:StyleID="s63" ss:MergeAcross="4">
                    <Data ss:Type="String">质量得分</Data>
                </Cell>
                <Cell ss:StyleID="s64" ss:MergeAcross="1">
                    <Data ss:Type="String">项目经理</Data>
                </Cell>
                <Cell ss:StyleID="s65" ss:MergeAcross="1">
                    <Data ss:Type="String">项目成员</Data>
                </Cell>
            </Row>
            <Row>
                <Cell ss:Index="11">
                    <Data ss:Type="String">P</Data>
                </Cell>
                <Cell>
                    <Data ss:Type="String">A</Data>
                </Cell>
                <Cell>
                    <Data ss:Type="String">N</Data>
                </Cell>
                <Cell>
                    <Data ss:Type="String">E</Data>
                </Cell>
                <Cell>
                    <Data ss:Type="String">L</Data>
                </Cell>
                <Cell ss:Index="18">
                    <Data ss:Type="String">姓名</Data>
                </Cell>
                <Cell>
                    <Data ss:Type="String">用时</Data>
                </Cell>
                <Cell>
                    <Data ss:Type="String">K值</Data>
                </Cell>
                <Cell>
                    <Data ss:Type="String">姓名</Data>
                </Cell>
                <Cell>
                    <Data ss:Type="String">用时</Data>
                </Cell>
                <Cell>
                    <Data ss:Type="String">K值</Data>
                </Cell>
                <Cell ss:Index="27">
                    <Data ss:Type="String">P</Data>
                </Cell>
                <Cell>
                    <Data ss:Type="String">A</Data>
                </Cell>
                <Cell>
                    <Data ss:Type="String">N</Data>
                </Cell>
                <Cell>
                    <Data ss:Type="String">E</Data>
                </Cell>
                <Cell>
                    <Data ss:Type="String">L</Data>
                </Cell>
                <Cell>
                    <Data ss:Type="String">贡献比例</Data>
                </Cell>
                <Cell>
                    <Data ss:Type="String">K值</Data>
                </Cell>
                <Cell>
                    <Data ss:Type="String">贡献比例</Data>
                </Cell>
                <Cell>
                    <Data ss:Type="String">K值</Data>
                </Cell>
            </Row>

            <#list departments as department>

                <Row>
                    <#if (department.interiors?size>0)>
                        <Cell ss:StyleID="s67" ss:MergeDown="${department.interiors?size}">
                            <Data ss:Type="String">${department.departmentName}</Data>
                        </Cell>
                    <#else >
                        <Cell ss:StyleID="s67" ss:MergeDown="1">
                            <Data ss:Type="String">${department.departmentName}</Data>
                        </Cell>
                        <Cell>
                            <Data ss:Type="String">无</Data>
                        </Cell>
                        <Cell>
                            <Data ss:Type="String">无</Data>
                        </Cell>
                        <Cell ss:StyleID="s50">
                            <Data ss:Type="String">无</Data>
                        </Cell>
                        <Cell>
                            <Data ss:Type="String">无</Data>
                        </Cell>
                        <Cell>
                            <Data ss:Type="String">无</Data>
                        </Cell>
                        <Cell>
                            <Data ss:Type="String">无</Data>
                        </Cell>
                        <Cell>
                            <Data ss:Type="String">无</Data>
                        </Cell>
                        <Cell>
                            <Data ss:Type="String">无</Data>
                        </Cell>
                        <Cell ss:StyleID="s54">
                            <Data ss:Type="String">无</Data>
                        </Cell>
                        <Cell>
                            <Data ss:Type="String">无</Data>
                        </Cell>
                        <Cell>
                            <Data ss:Type="String">无</Data>
                        </Cell>
                        <Cell>
                            <Data ss:Type="String">无</Data>
                        </Cell>
                        <Cell>
                            <Data ss:Type="String">无</Data>
                        </Cell>
                        <Cell>
                            <Data ss:Type="String">无</Data>
                        </Cell>
                        <Cell>
                            <Data ss:Type="String">无</Data>
                        </Cell>
                        <Cell>
                            <Data ss:Type="String">无</Data>
                        </Cell>
                        <Cell>
                            <Data ss:Type="String">无</Data>
                        </Cell>
                        <Cell>
                            <Data ss:Type="String">无</Data>
                        </Cell>
                        <Cell>
                            <Data ss:Type="String">无</Data>
                        </Cell>
                        <Cell>
                            <Data ss:Type="String">验收时间</Data>
                        </Cell>
                        <Cell>
                            <Data ss:Type="String">验收人</Data>
                        </Cell>
                        <Cell>
                            <Data ss:Type="String">无</Data>
                        </Cell>
                        <Cell>
                            <Data ss:Type="String">无</Data>
                        </Cell>
                        <Cell>
                            <Data ss:Type="String">无</Data>
                        </Cell>
                        <Cell>
                            <Data ss:Type="String">无</Data>
                        </Cell>
                        <Cell>
                            <Data ss:Type="String">无</Data>
                        </Cell>
                        <Cell>
                            <Data ss:Type="String">无</Data>
                        </Cell>
                        <Cell>
                            <Data ss:Type="String">无</Data>
                        </Cell>
                        <Cell>
                            <Data ss:Type="String">无</Data>
                        </Cell>
                    </#if>

                    <#list department.interiors as interior>
                        <Cell>
                            <Data ss:Type="String">任务序号</Data>
                        </Cell>
                        <Cell>
                            <Data ss:Type="String">${interior.projectNumber}</Data>
                        </Cell>
                        <Cell ss:StyleID="s50">
                            <Data ss:Type="String"><#--${interior.startTime?date("yyyy-MM-dd")}-->ss</Data>
                        </Cell>
                        <Cell>
                            <Data ss:Type="String">是</Data>
                        </Cell>
                        <Cell>
                            <Data ss:Type="String">${interior.projectName}</Data>
                        </Cell>
                        <Cell>
                            <Data ss:Type="String">${interior.initWorkload}</Data>
                        </Cell>
                        <Cell>
                            <Data ss:Type="String">${interior.projectManagerName}</Data>
                        </Cell>
                        <Cell>
                            <Data ss:Type="String">${interior.directReportName}</Data>
                        </Cell>
                        <Cell ss:StyleID="s54">
                            <Data ss:Type="String">${interior.newReport.progress}</Data>
                        </Cell>
                        <Cell>
                            <Data ss:Type="String">${interior.newReport.p}</Data>
                        </Cell>
                        <Cell>
                            <Data ss:Type="String">${interior.newReport.a}</Data>
                        </Cell>
                        <Cell>
                            <Data ss:Type="String">${interior.newReport.n}</Data>
                        </Cell>
                        <Cell>
                            <Data ss:Type="String">${interior.newReport.e}</Data>
                        </Cell>
                        <Cell>
                            <Data ss:Type="String">${interior.newReport.l}</Data>
                        </Cell>
                        <Cell>
                            <Data ss:Type="String">${interior.newReport.type}</Data>
                        </Cell>
                        <Cell>
                            <Data ss:Type="String">${interior.newReport.quality}</Data>
                        </Cell>
                        <#list interior.newReport.excelUserKs as userk>
                            <Cell>
                                <Data ss:Type="String">${userk.userName}</Data>
                            </Cell>
                            <Cell>
                                <Data ss:Type="String">${userk.time}</Data>
                            </Cell>
                            <Cell>
                                <Data ss:Type="String">${userk.k}</Data>
                            </Cell>
                        </#list>
                        <#list interior.weekAcceptances as weekAcceptance>
                            <Cell>
                                <Data ss:Type="String">验收时间</Data>
                            </Cell>
                            <Cell>
                                <Data ss:Type="String">验收人</Data>
                            </Cell>
                            <Cell>
                                <Data ss:Type="String">${weekAcceptance.progress}</Data>
                            </Cell>
                            <Cell>
                                <Data ss:Type="String">${weekAcceptance.p}</Data>
                            </Cell>
                            <Cell>
                                <Data ss:Type="String">${weekAcceptance.a}</Data>
                            </Cell>
                            <Cell>
                                <Data ss:Type="String">${weekAcceptance.n}</Data>
                            </Cell>
                            <Cell>
                                <Data ss:Type="String">${weekAcceptance.e}</Data>
                            </Cell>
                            <Cell>
                                <Data ss:Type="String">${weekAcceptance.l}</Data>
                            </Cell>
                            <#list weekAcceptance.weekAllocations as weekAllocation>
                                <Cell>
                                    <Data ss:Type="String">${weekAllocation.ratio}</Data>
                                </Cell>
                                <Cell>
                                    <Data ss:Type="String">${weekAllocation.k}</Data>
                                </Cell>
                            </#list>
                        </#list>
                    </#list>
                </Row>
            </#list>

        </Table>
        <WorksheetOptions xmlns="urn:schemas-microsoft-com:office:excel">
            <Selected/>
            <TopRowVisible>0</TopRowVisible>
            <LeftColumnVisible>11</LeftColumnVisible>
            <PageBreakZoom>100</PageBreakZoom>
            <Panes>
                <Pane>
                    <Number>3</Number>
                    <ActiveRow>14</ActiveRow>
                    <ActiveCol>28</ActiveCol>
                    <RangeSelection>R15C29</RangeSelection>
                </Pane>
            </Panes>
            <ProtectObjects>False</ProtectObjects>
            <ProtectScenarios>False</ProtectScenarios>
        </WorksheetOptions>
    </Worksheet>
    <Worksheet ss:Name="Sheet2">
        <Table ss:ExpandedColumnCount="1" ss:ExpandedRowCount="1" x:FullColumns="1" x:FullRows="1"
               ss:DefaultColumnWidth="54" ss:DefaultRowHeight="14.25"/>
        <WorksheetOptions xmlns="urn:schemas-microsoft-com:office:excel">
            <PageSetup>
                <Header x:Margin="0.511111111111111"/>
                <Footer x:Margin="0.511111111111111"/>
            </PageSetup>
            <TopRowVisible>0</TopRowVisible>
            <LeftColumnVisible>0</LeftColumnVisible>
            <PageBreakZoom>100</PageBreakZoom>
            <ProtectObjects>False</ProtectObjects>
            <ProtectScenarios>False</ProtectScenarios>
        </WorksheetOptions>
    </Worksheet>
    <Worksheet ss:Name="Sheet3">
        <Table ss:ExpandedColumnCount="1" ss:ExpandedRowCount="1" x:FullColumns="1" x:FullRows="1"
               ss:DefaultColumnWidth="54" ss:DefaultRowHeight="14.25"/>
        <WorksheetOptions xmlns="urn:schemas-microsoft-com:office:excel">
            <PageSetup>
                <Header x:Margin="0.511111111111111"/>
                <Footer x:Margin="0.511111111111111"/>
            </PageSetup>
            <TopRowVisible>0</TopRowVisible>
            <LeftColumnVisible>0</LeftColumnVisible>
            <PageBreakZoom>100</PageBreakZoom>
            <ProtectObjects>False</ProtectObjects>
            <ProtectScenarios>False</ProtectScenarios>
        </WorksheetOptions>
    </Worksheet>
</Workbook>