...
<div style="margin: 15px;">
    ...
    <form class="layui-form" action="" id="formData" ng-controller="editMonthDaysUserCtr" ng-cloak>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-inline">
                <input type="number" name="dayNum" value="{{monthDaysUser.dayNum}}" lay-verify="required|isNumber|isZero" placeholder="请输入天数" autocomplete="off" class="layui-input" maxlength="10">
            </div>
        </div>
...
<script>
    ...
    webApp.controller("editMonthDaysUserCtr", function($scope,$http,$timeout){
        $scope.monthDaysUser = JSON.parse(localStorage.getItem("monthDaysUser")); //目标配置信息
        console.log($scope.monthDaysUser);
        layui.use(['form', 'layedit', 'laydate'], function() {
            var form = layui.form(),
                    layer = layui.layer,
                    laydate = layui.laydate;

            getCompanyListType(null, $scope.monthDaysUser.companyId);
            getDepartmentListType($scope.monthDaysUser.companyId, null, null, $scope.monthDaysUser.departmentId);
            getDepartmentIdUser($scope.monthDaysUser.departmentId, $scope.monthDaysUser.userId);
            listForSelect($scope.monthDaysUser.monthDaysId);
            ...
            $("select[name='type'] option").each(function () {
                if ($(this).val() == $scope.monthDaysUser.type) {
            ...
            form.on('submit(demo1)', function(data) {
                data.field.id = $scope.monthDaysUser.id;
