...
<body ng-app="webApp" ng-controller="insideKnotAcceptanceListCtr" ng-cloak>
    <div style="margin: 15px;">
        <blockquote class="layui-elem-quote">内部结项情况&nbsp;&nbsp;&nbsp;<%--{{userInfo.id == aTeacherId}}，{{isDistribution}}，{{isAllocated}}--%>
    ...
    <script>
        ...
        webApp.controller("insideKnotAcceptanceListCtr", function($scope,$http,$timeout){
            ...
            YZ.ajaxRequestData("get", false, YZ.ip + "/projectAcceptanceRecord/list", {projectId : $scope.projectExternalId, type : 1}, null , function(result){
                if(result.flag == 0 && result.code == 200){
                    $scope.acceptanceList = result.data;
                    //获取可分配的ID
                    for (var i = 0; i < $scope.acceptanceList.length; i++) {
                        if ($scope.acceptanceList[i].status == 13 || $scope.acceptanceList[i].status == 12) { //表示已经有可以分配的数据了
            ...
            $scope.distribution = function () {
                layer.open({
                    ...
                    area: ['700px', '500px'],
                    content: YZ.ip + "/page/project/projectExternal/distributionInsideKnot?projectRecordId=" + $scope.projectRecordId +
