...
<div style="margin: 15px;">
    ...
    <form class="layui-form" action="" id="formData" ng-controller="userEditCtr" ng-cloak>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-inline" style="width: 500px;">
                <div class="roleInfo" id="roleInfo">
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-inline">
                <input disabled value="{{userInfo.account}}" type="text" lay-verify="required|isNumberChar" placeholder="请输入账号" autocomplete="off" class="layui-input" maxlength="16">
            </div>
        </div>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-inline">
                <input value="{{userInfo.name}}" type="text" name="name" lay-verify="required" placeholder="请输入姓名" autocomplete="off" class="layui-input" maxlength="20">
            </div>
        </div>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-inline">
                <input disabled value="{{userInfo.email}}" type="text" name="email" lay-verify="required|isEmail" autocomplete="off" class="layui-input" placeholder="请输入邮箱" maxlength="50">
            </div>
        </div>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-inline">
                <input value="{{ userInfo.birthday | date:'yyyy-MM-dd'}}" type="text" name="birthday" lay-verify="required" placeholder="yyyy-mm-dd" autocomplete="off" class="layui-input" onclick="layui.laydate({elem: this})" readonly>
            </div>
        </div>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-inline">
                <input value="{{userInfo.phone}}" type="tel" name="phone" lay-verify="phone" autocomplete="off" class="layui-input" placeholder="请输入电话号码" maxlength="11">
            </div>
        </div>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-inline">
                <input value="{{ userInfo.entryTime | date:'yyyy-MM-dd'}}" type="text" name="entryTime" id="entryTime" lay-verify="required" placeholder="yyyy-mm-dd" autocomplete="off" class="layui-input" readonly>
            </div>
        </div>

        <div class="layui-form-item">
            <label class="layui-form-label">转正时间<span class="font-red">*</span></label>
            <div class="layui-input-inline">
                <input value="{{ userInfo.positiveTime | date:'yyyy-MM-dd'}}" type="text" name="positiveTime" id="positiveTime" lay-verify="" placeholder="yyyy-mm-dd" autocomplete="off" class="layui-input" readonly>
            </div>
        </div>
        ...
        <div class="layui-form-item">
            <div class="layui-inline">
                ...
                <div class="layui-input-inline" style="width: 100px;">
                    <input value="{{userInfo.salary}}" type="text" name="salary" placeholder="￥" lay-verify="required|isDouble|isZero" onkeyup="YZ.clearNoNum(this)" onblur="YZ.clearNoNum(this)" autocomplete="off" class="layui-input" maxlength="10">
                </div>
            </div>
        </div>
    ...
    webApp.controller("userEditCtr", function($scope,$http,$timeout){
        ...
        layui.use(['layer', 'form', 'layedit', 'laydate', 'element'], function() {
            ...
            for (var i = 0; i < $scope.userInfo.roles.length; i++) {
                var obj = $scope.userInfo.roles[i];
                var json = {
                    id : obj.id,
                    roleName : obj.roleName
                }
                selectedRoleList.push(json);
                checkIds.push(obj.id);
            }

            getIsIframe(JSON.stringify(selectedRoleList), checkIds);
            ...
            $("input[name='sex']").each(function () {
                if (Number($(this).val()) == Number($scope.userInfo.sex)) {
                    $(this).click();
                }
            });

            //选中合作状态
            $("input[name='infoStatus']").each(function () {
                if (Number($(this).val()) == Number($scope.userInfo.infoStatus)) {
                    $(this).click();
                }
            });
            ...
            $("select[name='userType'] option").each(function () {
                if ($scope.userInfo.type == 0) {
                    if ($(this).val() == 0) {
                        ...
                        getRoleListType(0, $scope.userInfo.roleId);
                    }
                }
                else if($scope.userInfo.type == 1){
                    if ($(this).val() == 1) {
                        $(this).attr("selected", "selected");
                        getRoleListType(1, $scope.userInfo.roleId);
                    }
                }
            });

            if ($scope.userInfo.type == 1) {
                getCompanyListType(1, $scope.userInfo.companyId);
            }
            if ($scope.userInfo.type == 0) {
                getCompanyListType(0, $scope.userInfo.companyId);
                getDepartmentList(null, 0, $scope.userInfo.departmentId);
            }
            else {
                getDepartmentList($scope.userInfo.companyId, 1, $scope.userInfo.departmentId);
            }
            //默认选中身份属性
            $("select[name='incumbency'] option").each(function () {
                if ($(this).val() == $scope.userInfo.incumbency) {
                    $(this).attr("selected", "selected");
                }
            });
            ...
            form.on('select(userType)', function(data){
                ...
                getRoleList(Number(data.value), 0);
                form.render('select');
            });
            //公司过滤器
            form.on('select(company)', function(data){
                getDepartmentList(Number(data.value), 1, 0);
                form.render('select');
            });
            ...
            form.on('submit(demo1)', function(data) {
                ...
                if($scope.userInfo.roleId != data.field.roleId){
                    YZ.ajaxRequestData("get", false, YZ.ip + "/user/verifyWordDiary", {userId : $scope.userInfo.id}, null , function(result){
                        if(result.flag == 0 && result.code == 200 && result.data == 1){
                            layer.confirm('该员工下还有日志未审核，确定修改角色信息吗？', {
                                btn: ['确定','考虑一下'] //按钮
                            }, function(){
                                YZ.ajaxRequestData("post", false, YZ.ip + "/user/updateUser", data.field , null , function(result) {
                                    if (result.flag == 0 && result.code == 200) {
                                        layer.alert('修改用户成功.', {
