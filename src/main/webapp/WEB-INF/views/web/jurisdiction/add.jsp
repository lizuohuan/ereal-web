...
<script>
    ...
    function initData (roleId) {
        //监听select事件--用户类型过滤器
        YZ.ajaxRequestData("get", false, YZ.ip + "/menu/findAllMenu", {roleId : roleId} , null , function(result) {
            if (result.flag == 0 && result.code == 200) {
                ...
                for (var i = 0; i < menuList.length; i++) { //第一级
                    ...
                    if (menuList[i].child.length != 0) {
                        var oneChildList = menuList[i].child;
                        for (var j = 0; j < oneChildList.length; j++) { //第二级
                            var twoChildHtml = "";
                            if (oneChildList[j].child.length != 0) {
                                var twoChildList = oneChildList[j].child;
                                for (var k = 0; k < twoChildList.length; k++) { //第三级
                                    var threeChildHtml = "";
                                    if (twoChildList[k].child.length != 0) {
                                        var threeChildList = twoChildList[k].child;
                                        for (var n = 0; n < threeChildList.length; n++) { //第四级
                                            var fourInput = "";
                                            if (threeChildList[n].spread) {
                                                fourInput = '<input type="checkbox" value="' + threeChildList[n].id + '" title="' + threeChildList[n].title + '" lay-filter="checkbox" checked>';
                                            }
                                            else {
                                                fourInput = '<input type="checkbox" value="' + threeChildList[n].id + '" title="' + threeChildList[n].title + '" lay-filter="checkbox">';
                                            }
                                            threeChildHtml += '<li>' +
                                                    '<ul class="list-item">' +
                                                    '<li>' + fourInput + '</li>' +
                                    ...
                                    var threeInput = "";
                                    if (twoChildList[k].spread) {
                                        threeInput = '<input type="checkbox" value="' + twoChildList[k].id + '" title="' + twoChildList[k].title + '" lay-filter="checkbox" checked>';
                                    }
                                    else {
                                        threeInput = '<input type="checkbox" value="' + twoChildList[k].id + '" title="' + twoChildList[k].title + '" lay-filter="checkbox">';
                                    }
                                    twoChildHtml += '<li>' +
                                            '<ul class="list-item">' +
                                            '<li>' + threeInput + '</li>' +
                            ...
                            var twoInput = "";
                            if (oneChildList[j].spread) {
                                twoInput = '<input type="checkbox" value="' + oneChildList[j].id + '" title="' + oneChildList[j].title + '" lay-filter="checkbox" checked>';
                            }
                            else {
                                twoInput = '<input type="checkbox" value="' + oneChildList[j].id + '" title="' + oneChildList[j].title + '" lay-filter="checkbox">';
                            }
                            oneChildHtml += '<li>' +
                                    '<ul class="list-item">' +
                                    '<li>' + twoInput + '</li>' +
                    ...
                    if (menuList[i].spread) {
                        if (roleId == 1 && menuList[i].id == 46) {
                            oneInput = '<input type="checkbox" value="' + menuList[i].id + '" title="' + menuList[i].title + '" lay-filter="checkbox" parentCheckBox="true" checked disabled>';
    ...
    layui.use(['form', 'layedit', 'laydate'], function() {
        ...
        form.on('submit(demo1)', function(data) {
            ...
            console.log(data.field);
            YZ.ajaxRequestData("post", false, YZ.ip + "/roleUrl/addRoleUrl", data.field , null , function(result) {
                if (result.flag == 0 && result.code == 200) {
